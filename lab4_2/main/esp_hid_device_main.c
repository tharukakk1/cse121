...
void ble_hid_demo_task(void *pvParameters)
{
    ESP_LOGI(TAG, "Lab 4.2 Mouse Sequence Task Started.");
    
    while (1) {
        ESP_LOGI(TAG, "Moving mouse right...");
        // Move right: send +10 pixels on X axis, 0 on Y axis, 0 buttons, 0 wheel
        for (int i = 0; i < 30; i++) {
            send_mouse(0, 10, 0, 0);
            vTaskDelay(20 / portTICK_PERIOD_MS); // Smooth tracking steps
        }

        vTaskDelay(500 / portTICK_PERIOD_MS); // Brief pause before returning

        ESP_LOGI(TAG, "Moving mouse left...");
        // Move left: send -10 pixels on X axis
        for (int i = 0; i < 30; i++) {
            send_mouse(0, -10, 0, 0);
            vTaskDelay(20 / portTICK_PERIOD_MS);
        }

        // Lab 4.2 Core Requirement: Pause execution for 5 seconds 
        ESP_LOGI(TAG, "Pausing mouse movement for 5 seconds...");
